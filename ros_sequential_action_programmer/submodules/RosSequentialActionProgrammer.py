...
class RosSequentialActionProgrammer():
    ...
    def append_service_to_action_list(self, service_client: str, service_type :str = None , service_name = None) -> bool:
        if not service_type:
            ...
            if not test_obj:
                self.node.get_logger().error(f"Service type '{service_type}' is unknown to the system!")
                return False
            
        if service_type:
            self.action_list.append(ServiceAction(client=service_client, 
                                                    service_type = service_type,
                                                    node=self.node,
                                                    name=service_name))
            self.node.get_logger().info(f"Inserted Service for {service_client} at {len(self.action_list)-1}")
            return True
        else:
            self.node.get_logger().error(f"Service for client '{service_client}' could not be appended because service type does not exist or is unknown!")
            return False

    def append_service_to_action_list_at_index(self, service_client: str, index: int, service_type :str = None, service_name = None) -> bool:
        if not service_type:
            service_type = self.get_service_type_for_client(service_client)

        if not index > len(self.action_list):
           self.action_list.insert(index, ServiceAction(client=service_client, 
                                                    service_type = service_type,
                                                    node=self.node,
                                                    name=service_name))
    ...
    def get_service_req_from_dict(self, client:str, dict: dict, service_type = None) -> any:
        """
    ...
    def load_from_JSON(self, file_path)-> bool:
        ...
        if file_data:
            ...
            for index, item in enumerate(file_data["action_list"]):
                ...
                action_from_item = ServiceAction(service_client,
                                                    item['service_type'],
                                                    self.node,
                                                    item['name'])
                
                set_success = action_from_item.set_service_bool_identifier(item['error_identifier'])
                action_from_item.description = item ["description"]
                if not set_success:
                    return False
                
                self.action_list.append(action_from_item)
                set_success = self.process_action_dict_at_index(index=index,mode=SET_IMPLICIT_SRV_DICT, input_impl_dict=item["service_request"])
                if not set_success:
                    self.action_list.clear()
                    return False
    ...
    def save_to_JSON(self) -> bool:
        ...
        if (self.folder_path is not None) and (self.name is not None):
            ...
            for index, action in enumerate(self.action_list):
                ...
                service_dict["service_request"] = json.loads(json.dumps(action.service_req_dict_implicit))
    ...
    def execute_current_action(self) -> bool:
        try:
            ...
            self.append_action_log(index = self.current_action_index,
            ...
            if success_exec:
                self.node.get_logger().info(f"Action {self.get_current_action_name()} executed successfully!")
            else:
                raise Exception
    ...
    def get_copy_impl_srv_dict_at_index(self, index: int) -> collections.OrderedDict:
        ...
        """
        try:
            return_value = copy.deepcopy(self.action_list[index].service_req_dict_implicit)
            return return_value
        except Exception as e:
            return None
        
    def set_service_req_from_dict_at_index(self, index: int, dict: collections.OrderedDict) -> bool:
        """
    ...
    def get_service_responses_list(self)-> list[collections.OrderedDict]:
        ...
        for index, action in enumerate(self.action_list):
            if isinstance(action, ServiceAction):
                ...
                dictionary = {}
                process_dict = collections.OrderedDict()
                action_response_dict = message_to_ordereddict(action.get_empthy_service_response())
    ...
    def get_all_service_req_res_dict(self):
        ...
        for client in self.list_of_active_clients:
            try:
                request = ServiceAction.get_service_request(self.get_service_type_for_client(client))
                request_dict = json.loads(json.dumps(message_to_ordereddict(request)))

                response = ServiceAction.get_service_response(self.get_service_type_for_client(client))
                response_dict = json.loads(json.dumps(message_to_ordereddict(response)))

                service_dict[client] = {"service_type": self.get_service_type_for_client(client),
    ...
    def save_all_service_req_res_to_JSON(self) -> bool:
        ...
        try:
            ...
            self.node.get_logger().info("Json with all service requests and responses exported!")
            return True
        except FileNotFoundError as e:
            print(e)
            self.node.get_logger().error("Directory does not exist!")
    ...
    def get_recom_for_action_at_index__key(self, index: int, key: str) -> list:
        possible_list = self.get_possible_srv_res_fields_at_index(index=index, target_key=key)
        self.recommendations.append_to_recommendation({'Service Responses': possible_list})
        custom_recom_list = []
        for category in self.recommendations.get_recommendations():
            for cat_key, cat_list in category.items():
                list_of_items = []
                for item in cat_list:
                    set_possible = self.process_action_dicts_at_index_from_key(index=index, key=key,value=item,mode=CHECK_COMPATIBILITY_ONLY)
                    if set_possible:
    ...
    def get_possible_srv_res_fields_at_index(self, index: int, target_key:str) -> list:
        ...
        for ind in range(0,index):
            key_value_list: list = ServiceAction.get_key_value_pairs_from_dict(self.action_list[ind].default_service_res_dict)
            for item in key_value_list:
                # unfold key value pairs
                for key, value in item.items():
                    compatible = self.process_action_dicts_at_index_from_key(index=index,
                                                                                key=target_key,
                                                                                value=value,
                                                                                mode=CHECK_COMPATIBILITY_ONLY)
                    if compatible:
                        compatible_list.append(f'service_response.{ind}-{self.get_action_at_index(ind).name}.{key}')
        return compatible_list
        

    def convert_to_ordered_dict(self,d):
        if isinstance(d, dict):
            return OrderedDict((key, self.convert_to_ordered_dict(value)) for key, value in d.items())
        elif isinstance(d, list):
            return [self.convert_to_ordered_dict(item) for item in d]
        else:
            return d
    
    def process_action_dict_at_index(self, index: int, mode: int, input_impl_dict = None) -> bool:
        """
        ...
        if input_impl_dict is None:
            key_value_list: list = ServiceAction.get_key_value_pairs_from_dict(self.action_list[index].service_req_dict_implicit)
        else:
        ...
        try:
            ...
            for item in key_value_list:
                # unfold key value pairs
                for key, value in item.items():
                    process_success = self.process_action_dicts_at_index_from_key(index=index,
                                                                                key=key,
                                                                                value=value,
                                                                                mode=mode)
    ...
    def process_action_dicts_at_index_from_key(self, index: int, key: str, value:any, mode: int) -> bool:
        """
        ...
        try:
            set_success = False
            is_reference, error_in_reference, ref_index, ref_key  = self.check_for_reference(value)
            ...
            if is_reference:
                if mode == CHECK_COMPATIBILITY_ONLY or mode == SET_IMPLICIT_SRV_DICT:
                    check_value = self.get_action_at_index(ref_index).get_default_srv_res_value_from_key(key=ref_key)
                else:
                    value = self.get_action_at_index(ref_index).get_srv_res_value_from_key(key=ref_key)
            ...
            if mode == CHECK_COMPATIBILITY_ONLY:
                if isinstance(self.get_action_at_index(index),ServiceAction):
                    ...
                    set_success = test_action.set_srv_req_dict_value_from_key(key,check_value)
                    del test_action
            ...
            if mode == SET_SRV_DICT:
                if isinstance(self.get_action_at_index(index),ServiceAction):
                    set_success = self.get_action_at_index(index).set_srv_req_dict_value_from_key(path_key=key, new_value=value)
            
            if not set_success:
                if not mode == CHECK_COMPATIBILITY_ONLY:
                    self.node.get_logger().error(f"In {self.get_action_at_index(index).name}, failed at setting key '{str(key)}' to value: '{str(value)}'")
                    if is_reference:
                        self.node.get_logger().error(f"Action {ref_index}-{self.get_action_at_index(ref_index).name} might not have been executed yet!")
                return False
    ...
    def check_for_reference(self, input_value: any) -> (bool,int,str):
        ...
        if identifier[0] == 'service_response':
            ...
            if ref_action_name != self.get_action_at_index(ref_index).name or not ref_index:
    ...
    def save_action_sequence_log(self) -> bool:
        ...
        if (self.folder_path is not None) and (self.name is not None):
            ...
            try:
                ...
                with open(f"{file_path}/{self.name}_log_{str(export_time)}.json", 'w') as json_file:
